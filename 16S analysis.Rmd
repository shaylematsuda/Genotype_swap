...
bacBarPlot<-plot_bar(bac50,  fill="Phylum") +
  facet_wrap("genet.ID");bacBarPlot #genets_H_A where decisions need to be made: 20 (3, tube 67, 67b, 67c), ML4 (2, Acer272, 272b): Keep Acer 272b and Acer 67c highest read counts
...
bad.samples <-  c("Acer67", "Acer67b","Acer272")  #remove from phylo obj
...
```{r}
Bac.seq.c<-subset_samples(Bac.seq, type=="sample") #subset only Mcap
...
#adonis
  #between home nursery: 
  set.seed(30)
  adonis2(Bac.Froz.wu ~ Home.nursery, data = samp.df) # p=0.001
...
# Homogeneity of dispersion test  
  ...
  TukeyHSD(ball,  ordered = FALSE,conf.level = 0.95)
      #### CRF is different than all others in dispersion. UM diff than mote. 
  
   set.seed(30) # AWAY NO DIFF!
...
stat_ellipse(geom="polygon", alpha=1/6, aes(fill=Home.nursery)) +
  ggtitle("NMDS 16s") +
  theme_classic() +
  facet_wrap(~Away.nursery) #wrap by Away

#by genet and Away
ggplot(scores.b, aes(x = Away.nursery, y = NMDS2, fill=Away.nursery)) + 
  geom_point(aes(color=Away.nursery),size = 4) +
      #scale_color_manual(values=pal$hex)+
#stat_ellipse(geom="polygon", alpha=1/6, aes(fill=tag.id)) +
  ggtitle("NMDS 16s") +
  theme_classic()+
  facet_wrap(~genet.ID+Home.nursery)
...
#adonis by colony by sample location by branch/plate
  Branch <- subset_samples(Bac.Froz, samp_loc =="Branch"|samp_loc=="Plate") #Subset by colony 
