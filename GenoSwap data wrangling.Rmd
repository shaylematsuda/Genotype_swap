...
genSamp3$genet.ID<-gsub(" ","",as.character(genSamp3$genet.ID)) #remove all "-", " ", "'"
genSamp3$genet.ID<-gsub("'","",as.character(genSamp3$genet.ID)) #remove all "-", " ", "'"
genSamp3$collect<-"frozen" #new col to designate fresh collection

#Frozen - merge home location data
Frozen2<-merge(Frozen,genSamp3, by="tube.id", all.x=T) 
...
indexes<-read.csv("data/index_samp.csv")   #upload the sample ids
...
treefile<- read.tree("data/16Stake4/16S-pipeline_outputs/Results/postprocessing/unifrac/FastTree_100.nwk")
