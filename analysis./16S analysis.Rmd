...
```{r}
load("data/RData/Bac.seq_phyloseq2500new.RData") #load cleaned data 
...
bacBarPlot<-plot_bar(bac50,  fill="Phylum") +
  facet_wrap("frag.id");bacBarPlot 
      #genets_H_A where decisions need to be made: 20 (3, tube 67, 67b, 67c), ML4 (2,             Acer272, 272b): 
    #Keep Acer 272b, Acer 67c, Acer 122, Acer175, Acer68c - highest read counts in run. 
...
#Frozen
Bac.seq.c.frozen<-subset_samples(Bac.seq, Swap=="Frozen") #subset only coral
Bac.seq.c.frozen = prune_taxa(taxa_sums(Bac.seq.c.frozen) > 0, Bac.seq.c.frozen) #this removes any OTU with 0s
Bac.seq.c.sd.frozen <- as(sample_data(Bac.seq.c.frozen), "data.frame") #sample df

#Fresh
Bac.seq.c.fresh<-subset_samples(Bac.seq, Swap=="Fresh") #subset only coral
Bac.seq.c.fresh = prune_taxa(taxa_sums(Bac.seq.c.fresh) > 0, Bac.seq.c.fresh) #this removes any OTU with 0s
Bac.seq.c.sd.fresh <- as(sample_data(Bac.seq.c.fresh), "data.frame") #sample df
...
#shannon ####
  ...
  set.seed(30)
  aov.shannon = aov(diversity_shannon ~ Home.nursery*Away.nursery*genet.ID, data=alpha)
    summary(aov.shannon) # home p=0.0285, genet p=0.0002, home*away p=0.039
    
#means
shannon_mean <- ddply(alpha, c("Home.nursery","Away.nursery"), summarise, #summarize cell counts
...
shannon_plot<-ggplot(data=shannon_mean, aes(x=Home.nursery, y=mean, color=Home.nursery)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Home.nursery), size=4, show.legend = F)+
  ...
  #       panel.background = element_rect(colour = "black", size=1))+
   ...
   ggtitle("Shannon by home")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  theme(plot.title = element_text(size=20, face = "italic"))+
  facet_wrap(~Away.nursery);shannon_plot

#shannon by sample type #### 
 shannon_mean <- ddply(alpha, c("Home.nursery","Away.nursery", "Swap"), summarise, #summarize cell counts
                 N    = length(diversity_shannon[!is.na(diversity_shannon)]), #calculate the length of the data frame, excluding NA’s
                 mean = mean(diversity_shannon, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(diversity_shannon, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
#plot facet away
shannon_plot<-ggplot(data=shannon_mean, aes(x=Home.nursery, y=mean, color=Swap)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Swap), size=4, show.legend = F)+
  xlab("Home Nursery") + #Label the X Axis
    ylab("Shannon Diversity") + #Label the X Axis
  theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        axis.title=element_text(size=14,face="bold"), #Set axis format
        panel.border = element_blank(), #Set the border
  ...
  #       panel.background = element_rect(colour = "black", size=1))+
        theme(aspect.ratio=1)+
   ylab(expression(paste("Shannon Diversity"))) +
   ggtitle("Shannon by Away Nursery")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  theme(plot.title = element_text(size=20, face = "italic"))+
  facet_wrap(~Away.nursery);shannon_plot

#plot facet home
shannon_plot<-ggplot(data=shannon_mean, aes(x=Away.nursery, y=mean, color=Swap)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Swap), size=4, show.legend = F)+
  ...
  #       panel.background = element_rect(colour = "black", size=1))+
        theme(aspect.ratio=1)+
   ylab(expression(paste("Shannon Diversity"))) +
   ggtitle("Shannon by Home Nursery")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  theme(plot.title = element_text(size=20, face = "italic"))+
  facet_wrap(~Home.nursery);shannon_plot
...
#Richness ####
  hist(alpha$observed)
  set.seed(30)
  aov.observed = aov(observed ~ Home.nursery*Away.nursery*genet.ID, data=alpha)
    summary(aov.observed) # away p=0.038
    
#means
observed_mean <- ddply(alpha, c("Home.nursery","Away.nursery"), summarise, #summarize cell counts
...
observed_plot<-ggplot(data=observed_mean, aes(x=Home.nursery, y=mean, color=Home.nursery)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Home.nursery), size=4, show.legend = F)+
  xlab("Away Nursery") + #Label the X Axis
  ...
  #       panel.background = element_rect(colour = "black", size=1))+
   ...
   ggtitle("observed by Home")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  theme(plot.title = element_text(size=20, face = "italic"))+
  facet_wrap(~Away.nursery);observed_plot

#observed by sample type #### 
 observed_mean <- ddply(alpha, c("Home.nursery","Away.nursery", "Swap"), summarise, #summarize cell counts
                 N    = length(observed[!is.na(observed)]), #calculate the length of the data frame, excluding NA’s
                 mean = mean(observed, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(observed, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
#plot facet away
observed_plot<-ggplot(data=observed_mean, aes(x=Home.nursery, y=mean, color=Swap)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Swap), size=4, show.legend = F)+
  xlab("Home Nursery") + #Label the X Axis
    ylab("observed Diversity") + #Label the X Axis
  theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        axis.title=element_text(size=14,face="bold"), #Set axis format
        panel.border = element_blank(), #Set the border
  ...
  #       panel.background = element_rect(colour = "black", size=1))+
        theme(aspect.ratio=1)+
   ylab(expression(paste("observed Diversity"))) +
   ggtitle("observed by Away Nursery")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  theme(plot.title = element_text(size=20, face = "italic"))+
  facet_wrap(~Away.nursery);observed_plot

#plot facet home
observed_plot<-ggplot(data=observed_mean, aes(x=Away.nursery, y=mean, color=Swap)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Swap), size=4, show.legend = F)+
  xlab("Away Nursery") + #Label the X Axis
  ...
  #       panel.background = element_rect(colour = "black", size=1))+
        theme(aspect.ratio=1)+
   ylab(expression(paste("observed Diversity"))) +
   ggtitle("observed by Home Nursery")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  theme(plot.title = element_text(size=20, face = "italic"))+
  facet_wrap(~Home.nursery);observed_plot
...
#evenness ####
  ...
  set.seed(30)
  aov.evenness = aov(evenness_pielou ~ Home.nursery*Away.nursery*genet.ID, data=alpha)
    summary(aov.evenness) # home p<0.001, away p<0.001, genet p<0.001, home*away p<0.001
    
#means
evenness_mean <- ddply(alpha, c("Home.nursery","Away.nursery"), summarise, #summarize cell counts
...
evenness_plot<-ggplot(data=evenness_mean, aes(x=Home.nursery, y=mean, color=Home.nursery)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Home.nursery), size=4, show.legend = F)+
  ...
  #       panel.background = element_rect(colour = "black", size=1))+
   ...
   ggtitle("evenness by home")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  theme(plot.title = element_text(size=20, face = "italic"))+
  facet_wrap(~Away.nursery);evenness_plot

#evenness by sample type #### 
 evenness_mean <- ddply(alpha, c("Home.nursery","Away.nursery", "Swap"), summarise, #summarize cell counts
                 N    = length(evenness_pielou[!is.na(evenness_pielou)]), #calculate the length of the data frame, excluding NA’s
                 mean = mean(evenness_pielou, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(evenness_pielou, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
#plot facet away
evenness_plot<-ggplot(data=evenness_mean, aes(x=Home.nursery, y=mean, color=Swap)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Swap), size=4, show.legend = F)+
  xlab("Home Nursery") + #Label the X Axis
    ylab("evenness Diversity") + #Label the X Axis
  theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        axis.title=element_text(size=14,face="bold"), #Set axis format
        panel.border = element_blank(), #Set the border
  ...
  #       panel.background = element_rect(colour = "black", size=1))+
        theme(aspect.ratio=1)+
   ylab(expression(paste("evenness Diversity"))) +
   ggtitle("evenness by Away Nursery")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  theme(plot.title = element_text(size=20, face = "italic"))+
  facet_wrap(~Away.nursery);evenness_plot

#plot facet home
evenness_plot<-ggplot(data=evenness_mean, aes(x=Away.nursery, y=mean, color=Swap)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Swap), size=4, show.legend = F)+
  xlab("Away Nursery") + #Label the X Axis
  ...
  #       panel.background = element_rect(colour = "black", size=1))+
        theme(aspect.ratio=1)+
   ylab(expression(paste("evenness Diversity"))) +
   ggtitle("evenness by Home Nursery")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  theme(plot.title = element_text(size=20, face = "italic"))+
  facet_wrap(~Home.nursery);evenness_plot
...
#Plots by AWAY ####
#plot together  
alpha.frozen<-subset(alpha, Swap=="Frozen")
...
observed_plot<-ggplot(data=observed_mean, aes(x=Home.nursery, y=mean, color=Home.nursery)) +
  ...
  #       panel.background = element_rect(colour = "black", size=1))+
   ...
   ggtitle("Richness")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  theme(plot.title = element_text(size=20, face = "italic"))+
  facet_wrap(~Away.nursery, nrow=1);observed_plot

#Shannon
#means
shannon_mean <- ddply(alpha.frozen, c("Home.nursery","Away.nursery"), summarise, #summarize cell counts
                 N    = length(diversity_shannon[!is.na(diversity_shannon)]), #calculate the length of the data frame, excluding NA’s
                 mean = mean(diversity_shannon, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(diversity_shannon, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
shannon_plot<-ggplot(data=shannon_mean, aes(x=Home.nursery, y=mean, color=Home.nursery)) +
  ...
  #       panel.background = element_rect(colour = "black", size=1))+
   ...
   ggtitle("Shannon Diversity")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  theme(plot.title = element_text(size=20, face = "italic"))+
  facet_wrap(~Away.nursery, nrow=1);shannon_plot

# EVENNESS means
evenness_mean <- ddply(alpha.frozen, c("Home.nursery","Away.nursery"), summarise, #summarize cell counts
                 N    = length(evenness_pielou[!is.na(evenness_pielou)]), #calculate the length of the data frame, excluding NA’s
                 mean = mean(evenness_pielou, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(evenness_pielou, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
evenness_plot<-ggplot(data=evenness_mean, aes(x=Home.nursery, y=mean, color=Home.nursery)) +
  ...
  geom_point(aes(color=Home.nursery), size=4, show.legend = F)+
  xlab("Home Nursery") + #Label the X Axis
    ylab("Evenness") + #Label the X Axis
  theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        axis.title=element_text(size=14,face="bold"), #Set axis format
        panel.border = element_blank(), #Set the border
  ...
  #       panel.background = element_rect(colour = "black", size=1))+
   ...
   ggtitle("Evenness")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  theme(plot.title = element_text(size=20, face = "italic"))+
  facet_wrap(~Away.nursery, nrow = 1);evenness_plot

#plot all together
grid.arrange( observed_plot, shannon_plot,evenness_plot, nrow = 3)
...
#regular box  
#evenness
  aov.evenness.species = aov(evenness_pielou ~ tag.id, data=alpha)
  summary(aov.evenness.species) # sig diff between species
  TukeyHSD(aov.evenness.species)  

  #Plot
  boxplot(diversity_observed ~ location, data=alpha, ylab="observed's diversity") 
  boxplot(observed ~ location, data=alpha, ylab="Observed") 
  boxplot(evenness_pielou ~ location, data=alpha, ylab="Evenness_pielou") 

#violin plots
  # create a list of pairwise comaprisons
al.species <- levels(alpha$location) # get the variables
...
#observed
p1 <- ggviolin(alpha, x = "location", y = "observed",
   add = "boxplot", fill = "location") 
  print(p1)

    p1 <- ggviolin(alpha, x = "tag.id", y = "observed",
   add = "boxplot", fill = "location") 
  print(p1)

  #observed
p1 <- ggviolin(alpha, x = "location", y = "diversity_observed",
   add = "boxplot", fill = "location") 
  print(p1)
...
#Evenness
p1 <- ggviolin(alpha, x = "location", y = "evenness_pielou",
   add = "boxplot", fill = "location") 
  print(p1)
