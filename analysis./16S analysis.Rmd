...
#shannon ####
  ...
  aov.shannon = aov(diversity_shannon ~ Home.nursery*Away.nursery*genet.ID, data=alpha)
    summary(aov.shannon) # only Home location matters (p=0.0285)
  aov.shannon = aov(diversity_shannon ~ Home.nursery, data=alpha) #try only by home
    summary(aov.shannon) 
    TukeyHSD(aov.shannon)# nothing significant
  aov.shannon = aov(diversity_shannon ~ genet.ID, data=alpha) #not significant
    summary(aov.shannon) 
    
    #look at residuals - nor running
   # car::qqPlot(residuals(aov.shannon))
    #car::leveneTest(residuals(aov.shannon)~Home.nursery, data=alpha) #not
    
  shannon_mean <- ddply(alpha, c("Home.nursery"), summarise, #summarize cell counts
...
#observed   ####
   hist(alpha$observed) #heavily skewed
  ggqqplot(alpha$observed)
  
  alpha$observed_log<-log10(alpha$observed)
  hist(alpha$observed_log) #good
  
  set.seed(30)
    aov.observed.species = aov(observed_log ~ tag.id*location*Prop.D, data=alpha)
      summary(aov.observed.species) # only location sig
  
  set.seed(30)
   aov.observed.species = aov(observed_log ~ date, data=alpha)
    summary(aov.observed.species) #date not important
  
  set.seed(30) #run for tukey
   aov.observed.species = aov(observed ~ location, data=alpha)
  ...
  TukeyHSD(aov.observed.species)# surprise, branch tips sig differ from all other locations
    ...
    car::leveneTest(residuals(aov.observed.species)~location, data=alpha) #passes

  observed_mean <- ddply(alpha, c("location"), summarise, #summarize cell counts
...
observed_plot<-ggplot(data=observed_mean, aes(x=location, y=mean, color=location)) +
 ...
 hist(alpha$evenness_pielou)
  ggqqplot(alpha$evenness_pielou)
   hist(alpha$evenness_pielou_log10)#hist of data looks best of all transformations but not great. 

  set.seed(30)
    aov.evenness_pielou.species = aov(evenness_pielou ~ tag.id*location*Prop.D, data=alpha)
      summary(aov.evenness_pielou.species) # Tag ID and location significant
  
    car::qqPlot(residuals(aov.evenness_pielou.species))
    car::leveneTest(residuals(aov.evenness_pielou.species)~location, data=alpha) #fails

    #transformed data also fails, stick with original data (sqrt fails, log fails, in fails, arcsqrt fails)
    
  aov.evenness_pielou.species = aov(evenness_pielou ~ date, data=alpha)
  summary(aov.evenness_pielou.species) #not sig
  
  aov.evenness_pielou.species = aov(evenness_pielou ~ location, data=alpha)
  summary(aov.evenness_pielou.species) # sig
  TukeyHSD(aov.evenness_pielou.species)#

  aov.evenness_pielou.species = aov(evenness_pielou ~ tag.id, data=alpha)
  summary(aov.evenness_pielou.species) # p < 0.001
  TukeyHSD(aov.evenness_pielou.species)# 
  
  aov.evenness_pielou.species = aov(evenness_pielou ~ Prop.D, data=alpha)
  summary(aov.evenness_pielou.species) # p =.43
  
  evenness_pielou_mean <- ddply(alpha, c("location"), summarise, #summarize cell counts
...
#evenness
  ...
  #Plot
  boxplot(diversity_shannon ~ location, data=alpha, ylab="Shannon's diversity") 
...
p1 <- ggviolin(alpha, x = "location", y = "observed",
  ...
  #Shannon
p1 <- ggviolin(alpha, x = "location", y = "diversity_shannon",
   add = "boxplot", fill = "location") 
  print(p1)
